...
export default class RemedioAnimation extends EscondidoAnimation {
	constructor(id) {
		...
		this.elements = {
			...
			giroCoronacion: new RotatePairElement(this, {
				left: remedioPaths.coronacion_left,
				right: remedioPaths.coronacion_right
			}, 360),
	...
	esquina(lengthS, manPosition, beats) {
		this.clearPaths();
		const leftPaths = {
			esquinaPath: remedioPaths.esquina_left,
			angle: 225
		};
		const topPaths = {
			esquinaPath: remedioPaths.esquina_top,
			angle: 315
		};
		const rightPaths = {
			esquinaPath: remedioPaths.esquina_right,
			angle: 45
		};
		const bottomPaths = {
			esquinaPath: remedioPaths.esquina_bottom,
			angle: 135
		};
		var manPaths;
		var womanPaths;
